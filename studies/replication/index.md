    ...
    <script>
      ...
      const renderList1 = (data, filter = '') => {
        ...
        for (let file of data.filter(filterFiles1(filter))) {
          ...
          const fileName = file.path.replace('.html', '').toLowerCase().replace(/-/g, ' ').replace('analysis/', '');
          htmlString += `<li><a href="https://bradkennedy-phd.github.io/KnobeReplication/${filePath}" target="_blank" rel="noopener noreferrer">R Markdown of Analysis /a><i class="fas fa-external-link-alt"></i></li>`;
        }
      htmlString += '</ul>';
